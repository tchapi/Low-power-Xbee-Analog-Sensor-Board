...
void setup() {
  ...
  // Wait for association with base
  delay(6000);
  
  // Send init message
  String init_msg = "Sensor " + String(DATASET_ID) + String(SENSOR_ID) + " is inited.\n";
  XBee.print(init_msg);
  XBee.print("\r");
  delay(750);
...
void loop() {
   ...
   */
   ...
   int temp = analogRead(AIO1);
   temp *= 1.07;
   temp -= 500; /* Actually it is temp*10 */

   logs[logcount].dataset = DATASET_ID;
   logs[logcount].sensor_id = SENSOR_ID;
